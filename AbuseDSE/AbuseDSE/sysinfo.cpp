...
static VOID PrintGuid(_In_ GUID Guid)
{
    Printf(L"{%08lx-%04hx-%04hx-%02hhx%02hhx-%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx}", Guid.Data1, Guid.Data2, Guid.Data3,
...
PrintSystemInformation()
{
    ...
    else
    {
        PRTL_PROCESS_MODULES ModuleInfo =
            static_cast<PRTL_PROCESS_MODULES>(RtlAllocateHeap(RtlProcessHeap(), HEAP_ZERO_MEMORY, 2 * Size));
        ...
        else
        {
            ...
            Printf(L"SystemModuleInformation:\n\t- Kernel: %S (%S)\n\n",
                   reinterpret_cast<PCHAR>(Ntoskrnl.FullPathName + Ntoskrnl.OffsetToFileName), Ntoskrnl.FullPathName);
        }
    ...
    Status =
        NtQuerySystemInformation(SystemCodeIntegrityInformation, &CodeIntegrityInfo, sizeof(CodeIntegrityInfo), nullptr);
    if (!NT_SUCCESS(Status))
    ...
    Status =
        NtQuerySystemInformation(SystemKernelDebuggerInformation, &KernelDebuggerInfo, sizeof(KernelDebuggerInfo), nullptr);
    if (!NT_SUCCESS(Status))
    ...
    if ((RtlNtMajorVersion() >= 6 && RtlNtMinorVersion() >= 3) || RtlNtMajorVersion() > 6)
    {
        ...
        Status = NtQuerySystemInformation(SystemKernelDebuggerInformationEx, &KernelDebuggerInfoEx,
                                          sizeof(KernelDebuggerInfoEx), nullptr);
        if (!NT_SUCCESS(Status))
            Printf(L"SystemKernelDebuggerInformationEx: error %08X\n\n", Status);
        else
            Printf(
                L"SystemKernelDebuggerInformationEx:\n\t- DebuggerAllowed: %u\n\t- DebuggerEnabled: %u\n\t- "
                L"DebuggerPresent: %u\n\n",
                KernelDebuggerInfoEx.DebuggerAllowed, KernelDebuggerInfoEx.DebuggerEnabled,
                KernelDebuggerInfoEx.DebuggerPresent);
    }
    ...
    if (RtlNtMajorVersion() > 6)
    {
        ...
        Status =
            NtQuerySystemInformation(SystemKernelDebuggerFlags, &KernelDebuggerFlags, sizeof(KernelDebuggerFlags), nullptr);
        if (!NT_SUCCESS(Status))
        ...
        Status = NtQuerySystemInformation(SystemCodeIntegrityPolicyInformation, &CodeIntegrityPolicyInfo,
                                          sizeof(CodeIntegrityPolicyInfo), nullptr);
        if (!NT_SUCCESS(Status))
        ...
        Status = NtQuerySystemInformation(SystemKernelDebuggingAllowed, &KernelDebuggingAllowed, 0, nullptr);
        ...
        Status = NtQuerySystemInformation(SystemCodeIntegrityUnlockInformation, &CodeIntegrityUnlockInfos[0],
                                          sizeof(CodeIntegrityUnlockInfos), nullptr);
        if (!NT_SUCCESS(Status))
        ...
        else
        {
            ...
            for (ULONG i = 0; i < ARRAYSIZE(CodeIntegrityUnlockInfos); ++i)
            {
                ...
                Printf(
                    L"SystemCodeIntegrityUnlockInformation:\n\t- Locked: %u"
                    L"\n\t- Unlockable: %u\n\t- UnlockApplied: %u\n\t- Flags: %04X\n\n",
